...
export default class ProfileVisiteur extends Component {
            ...
            this.setState({loadingData1: true}, () => {
                ...
                this.setState({
                    ...
                    if (this.state.activeIndex === 0) {
                        this.loadDataPublicationScroll(`https://api.trippybook.com/api/visiteur/publications/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, 'feeds')
                    } else if (this.state.activeIndex === 1) {
                        this.loadDataPublicationScroll(`https://api.trippybook.com/api/visiteur/photos/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, 'photos')
                    } else if (this.state.activeIndex === 2) {
                        this.loadDataPublicationScroll(`https://api.trippybook.com/api/visiteur/videos/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, 'videos')
    ...
    loadDataPublicationHome = async () => {
        try {
            const response = await axios.get(`https://api.trippybook.com/api/visiteur/publications/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, {headers: {'Authorization': this.state.token}});
            if (response.status === 200) {
                this.setState({
    ...
    loadDataPublicationPhoto = async () => {
        try {
            const response = await axios.get(`https://api.trippybook.com/api/visiteur/photos/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, {headers: {'Authorization': this.state.token}});
            if (response.status === 200) {
                this.setState({
    ...
    loadDataPublicationVideo = async () => {
        try {
            const response = await axios.get(`https://api.trippybook.com/api/visiteur/videos/visiteur/${this.props.navigation.state.params.idVisiteur}?page=${this.state.page}`, {headers: {'Authorization': this.state.token}});
            if (response.status === 200) {
                this.setState({
    ...
    loadDataShow = async () => {
        ...
        try {
            const response = await axios.get(`https://api.trippybook.com/api/visiteur/show/` + this.props.navigation.state.params.idVisiteur, {headers: {'Authorization': this.state.token}});
            if (response.status === 200) {
                console.log('loadDataShow ' + JSON.stringify(response.data));
                this.setState({loadDataShow: response.data}, () => {
                        this.setState({loadingData2: false})
                    }
                )
    ...
    componentDidMount() {
        ...
        setTimeout(() => this.isMyfriend(), 1200)
    ...
    unfriend = async () => {
        try {
            const res = await axios.post('https://api.trippybook.com/api/visiteur/amis/supprimerAmis', {visiteur_id: this.props.navigation.state.params.idVisiteur}, {headers: {'Authorization': this.state.token}})
            if (res.status == 200) {
                this.isMyfriend()
    ...
    friend = async () => {
        try {
            ...
            const res = await axios.post('https://api.trippybook.com/api/visiteur/amis/ajouterAmis', {
                receiver_id: this.props.navigation.state.params.idVisiteur,
                visiteur_id: this.state.idCurrentUser
    ...
    isMyfriend = async () => {
        const res = await axios.post('https://api.trippybook.com/api/visiteur/amis/friendStatus', {visiteur_id: this.props.navigation.state.params.idVisiteur}, {headers: {'Authorization': this.state.token}})
        if (res.status == 200) {
            if (res.data.friendStatus === "0") {
                this.setState({invitEnAttente: true})
            } else {
                this.setState({invitEnAttente: false, friendStatus: res.data.friendStatus})
        ...
        return (
            <View style={{flex: 1, flexDirection: 'column'}}>
                ...
                <Container style={{
                        ...
                        <Body>
                            <TouchableOpacity>
                                <Text style={{
                                    ...
                                    color: colors.black,
                                    fontWeight: 'bold'
                                }}>{!loadDataShow.visiteur ? null : loadDataShow.visiteur.nom + ' ' + loadDataShow.visiteur.prenom}</Text>
                            </TouchableOpacity>
                        </Body>
                        <Right>
                            <TouchableOpacity onPress={this.notif}><Text> <Icon name="bell" style={{
                                paddingRight: 10,
                                fontSize: 25
                    ...
                    <View>{(showList && loadDataCurrentUser.notifications) &&
                        ...
                        <Content>
                            <View style={{flexDirection: 'row', paddingTop: 10}}>
                                <View style={{flex: 1, alignItems: 'center'}}>
                                    {loadDataShow.visiteur != null ?
                                        <Image
                                            style={styles.containerAvatar}
                                            source={{uri: 'https://ressources.trippybook.com/assets/' + loadDataShow.visiteur.photo}}
                                        />:null}
                                </View>
                                <View style={{flex: 2, paddingHorizontal: 10, paddingVertical: 10}}>
                                    ...
                                    <Text
                                        style={styles.userEmail}>{!loadDataShow.visiteur ? null : loadDataShow.visiteur.email}</Text>
                                    <Text style={{fontSize: 15}}><Icon name={'map-marker'}
                                                                       style={{fontSize: 14}}/> {!loadDataShow.visiteur ? null : loadDataShow.visiteur.adresse}
                                    </Text>
                                    <Text style={{fontSize: 15}}><Icon name={'phone'}
                                                                       style={{fontSize: 14}}/> {!loadDataShow.visiteur ? null : loadDataShow.visiteur.tel}
                                    </Text>
                                </View>
                            </View>
                            <View style={{flex: 3}}>
                                ...
                                <View style={{flexDirection: 'row', paddingHorizontal: 20}}>
                                    <Button bordered dark onPress={() =>
                                        this.state.invitEnAttente ? this.unfriend() : this.state.friendStatus ? this.unfriend() : this.friend()}
                                            style={{
                                                flex: 1,
                                                marginLeft: 10,
                                        ...
                                        }}>{this.state.invitEnAttente ? 'Cancel' : this.state.friendStatus ? 'Unfriend' : 'Add friend'}</Text>
                                    </Button>
